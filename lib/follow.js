...
(function main(...args) {
  ...
  if (!location.host.includes("instagram"))
    throw new Error("You must be on instagram website");
  if (!location.pathname.includes("followers"))
    throw new Error("You must be on instagram followers account page");
  ...
  function follow(startAtIndex) {
    const waitBeforeCallFn = Math.floor(Math.random() * (10 - 5 + 1) + 5) * 1000;
    const getBtn = [...document.getElementsByClassName("sqdOP  L3NKy   y3zKF     ")];
    const arrayParsed = getBtn.slice(spliceAt);

    const indexToStart = startAtIndex || 0;
    let returnIndex = 0;
    let loopBreak = false;

    loop1: for (let index = indexToStart; index < arrayParsed.length; index++) {
      const [pseudo, ...other] =
